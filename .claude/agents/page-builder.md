...
4. Подбери имя ветки выпуска `<branch>`. По умолчанию — `digest/YYYY-MM-DD`. Если такая ветка уже существует (проверь локально через `git rev-parse --verify` и на origin через `git ls-remote --heads origin`), добавь числовой суффикс: `digest/YYYY-MM-DD-2`, `-3` и т.д., пока не получишь свободное имя. Несколько выпусков в один день — норма. **Ветку всегда создавай от свежего `main`, а не от текущей ветки** — иначе чужие незамерженные коммиты (например, WIP в `feat/...`) утянутся в выпуск и попадут в PR. Сначала подтяни `main`, потом ответвись от него:
   ```
   git fetch origin
